...
const DoctorImagePreprocessingTab: React.FC<DoctorImagePreprocessingTabProps> = ({ }) => {
    ...
    const processImage = () => {
        ...
        img.onload = () => {
            ...
            if (settings.edgeEnhance > 0) {
                applyEdgeEnhance(ctx, canvas.width, canvas.height, settings.edgeEnhance);
            }
    ...
    const applyEdgeEnhance = (ctx: CanvasRenderingContext2D, width: number, height: number, amount: number) => {
        const imageData = ctx.getImageData(0, 0, width, height);
        const data = imageData.data;
        const copy = new Uint8ClampedArray(data);
            ...
            for (let x = 1; x < width - 1; x++) {
                ...
                for (let ky = -1; ky <= 1; ky++) {
                    for (let kx = -1; kx <= 1; kx++) {
                        ...
                        const kxIdx = kx + 1;
                        const kyIdx = ky + 1;
                        gx += copy[idx] * sobelX[kyIdx * 3 + kxIdx];
                        gy += copy[idx] * sobelY[kyIdx * 3 + kxIdx];
                    }
                }
                ...
                // 混合原始图像和边缘检测结果
                const edgeAmount = amount / 10;
                data[idx] = Math.min(255, Math.max(0, copy[idx] * (1 - edgeAmount) + magnitude * edgeAmount));
                data[idx + 1] = Math.min(255, Math.max(0, copy[idx + 1] * (1 - edgeAmount) + magnitude * edgeAmount));
                data[idx + 2] = Math.min(255, Math.max(0, copy[idx + 2] * (1 - edgeAmount) + magnitude * edgeAmount));
            }
        }
                ...
                <aside className={styles.controlPanel}>
                    ...
                    <div className={styles.section}>
                        ...
                        <div className={styles.controlGroup}>
                            <div className={styles.controlLabel}>
                                <span>边缘增强</span>
                                <span className={styles.value}>{settings.edgeEnhance}</span>
                            </div>
                            <input
                                type="range"
                                min="0"
                        ...
                        {processedImage ? (
                            <>
                                ...
                                <div className={styles.imageInfo}>
                                    ...
                                    <div className={styles.infoItem}>
                                        <Contrast size={16} />
                                        <span>对比度: {settings.contrast}%</span>
                                    </div>
                                </div>
                            </>
                        ) : (
                            <div className={styles.placeholder}>
                                <ImageIcon size={48} />
                                <p>请上传医学图像开始处理</p>
                                <small>支持 X光片、CT、MRI 等医学图像格式</small>
                            </div>
                        )}
                    </div>
