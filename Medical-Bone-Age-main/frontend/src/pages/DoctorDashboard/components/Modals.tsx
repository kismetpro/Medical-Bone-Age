...
interface PredictionModalProps {
  ...
  setPredictionForm: React.Dispatch<
    ...
    <div className={styles.modalOverlay}>
      <div className={styles.modalContent}>
        <div className={styles.modalHeader}>
          <h3>新增预测记录</h3>
          <button className={styles.closeBtn} onClick={closePredictionModal}>
            <X size={20} />
          </button>
        </div>
            ...
            <div>
              <div className={styles.detailBlock}>
                ...
                <select
                  ...
                  onChange={(event) =>
                    setPredictionForm((previous) => ({ ...previous, targetUserId: event.target.value }))
                  }
                >
              ...
              <div className={styles.detailBlock}>
                ...
                <div className={styles.genderSwitch}>
                  <button
                    type="button"
                    className={predictionForm.gender === 'male' ? styles.genderSwitchActive : ''}
                    onClick={() => setPredictionForm((previous) => ({ ...previous, gender: 'male' }))}
                  >
                    男
                  </button>
                  <button
                    type="button"
                    className={predictionForm.gender === 'female' ? styles.genderSwitchActive : ''}
                    onClick={() => setPredictionForm((previous) => ({ ...previous, gender: 'female' }))}
                  >
                    女
                  </button>
                </div>
              ...
              <div className={styles.predictionInputGrid}>
                <input
                  className={styles.formInput}
                  placeholder="当前身高（cm，可选）"
                  value={predictionForm.currentHeight}
                  onChange={(event) =>
                    setPredictionForm((previous) => ({ ...previous, currentHeight: event.target.value }))
                  }
                />
                <input
                  className={styles.formInput}
                  placeholder="实际年龄（岁，可选）"
                  value={predictionForm.realAge}
                  onChange={(event) =>
                    setPredictionForm((previous) => ({ ...previous, realAge: event.target.value }))
                  }
                />
              </div>
                    ...
                    <input
                      ...
                      onChange={(event) =>
                        setPredictionForm((previous) => ({
                          ...previous,
                          preprocessingEnabled: event.target.checked,
                        }))
                      }
                    />
              ...
              <div className={styles.modalFooter}>
                <button className={styles.actionBtn} onClick={closePredictionModal}>
                  取消
                </button>
                <button
            ...
            <div>
              <div
                ...
                onDrop={(event) => {
                  ...
                  if (file) loadPredictionFile(file);
                }}
              >
                {predictionPreview ? (
                  <img src={predictionPreview} alt="预测预览" className={styles.uploadPreviewImage} />
                ) : (
                  <div className={styles.uploadPlaceholder}>
                    <Upload size={22} />
                    <p>点击或拖拽上传影像文件</p>
                    <span>直接复用个人用户预测评估流程</span>
                  </div>
                )}
              </div>
              <input
...
export const DetailModal: React.FC<DetailModalProps> = ({ selectedRecord, setSelectedRecord }) => {
  const genderLabel = (value: string) => (value === 'male' ? '男' : '女');
  const foreignObjectDetection = resolveForeignObjectDetection(selectedRecord);
            ...
            <div>
              <div className={styles.detailBlock}>
                ...
                <p>
                  ...
                  {selectedRecord.user_id ? ` (UID: ${selectedRecord.user_id})` : ''}
                </p>
              </div>
              <div className={styles.detailBlock}>
                <h4>预测骨龄</h4>
              ...
              <div className={styles.detailBlock}>
                <h4>性别</h4>
                <p>{genderLabel(selectedRecord.gender)}</p>
              </div>
              {selectedRecord.real_age_years != null && (
                <div className={styles.detailBlock}>
                  <h4>实际年龄</h4>
                  <p>{selectedRecord.real_age_years.toFixed(1)} 岁</p>
                </div>
              )}
              {selectedRecord.predicted_adult_height != null && (
                <div className={styles.detailBlock}>
                  <h4>预测成年身高</h4>
                  <p>{selectedRecord.predicted_adult_height.toFixed(1)} cm</p>
                </div>
              )}
              <div className={styles.detailBlock}>
                <h4>异物检测</h4>
                {foreignObjectDetection.detected ? (
                  <>
                    <p style={{ color: '#c2410c', fontWeight: 700 }}>{foreignObjectDetection.message}</p>
                    <p style={{ color: '#9a3412' }}>{`${foreignObjectDetection.count} 处高置信度异物`}</p>
                  </>
                ) : (
                  <p style={{ color: '#16a34a' }}>未检测到明显异物。</p>
                )}
              </div>
              <div className={styles.detailBlock}>
                <h4>异常特征</h4>
                  ...
                  <ul style={{ color: '#ef4444', margin: '0 0 1rem 0', paddingLeft: '1.2rem' }}>
                    {selectedRecord.anomalies.map((anomaly, index) => (
                      <li key={`${anomaly.type}-${index}`}>
                        {getAnomalyDisplayName(anomaly.type)} ({(anomaly.score * 100).toFixed(0)}%)
                      </li>
                    ))}
                  </ul>
                ) : (
                    ...
                    {selectedRecord.anomalies?.map((item, index) =>
                      item.score >= ANOMALY_ALERT_THRESHOLD ? (
                        <div
                          key={`${item.type}-${index}`}
                          style={{
                            left: `${(item.coord[0] - item.coord[2] / 2) * 100}%`,
                            top: `${(item.coord[1] - item.coord[3] / 2) * 100}%`,
                            width: `${item.coord[2] * 100}%`,
                            height: `${item.coord[3] * 100}%`,
                            position: 'absolute',
                            border: '2px solid #ef4444',
                            pointerEvents: 'none',
                          }}
                        >
                          <span className={styles.anomalyLabel}>{getAnomalyDisplayName(item.type)}</span>
                        </div>
                      ) : null,
                    )}
                  </div>
